...
def get_cached_price() -> Optional[Dict[str, Any]]:
    """Return cached BTC price if it is still fresh, else None."""
    ttl = app.config["CACHE_TTL_SECONDS"]
    now = time.time()
    if _price_cache["data"] is None:
        return None
    if now - _price_cache["fetched_at"] > ttl:
        # Cache expired
        return None
...
def get_btc_price():
    ...
    if cached:
        logger.debug("Serving BTC price from cache.")
    ...
    try:
        ...
        logger.error(
            ...
            extra={"provider": "coingecko", "error": str(e), "status": "error"},
        )
        return (
            jsonify(
                {
